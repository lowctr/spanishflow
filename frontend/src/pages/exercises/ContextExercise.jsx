...
export default function ContextExercise({ word, nextWord, onNext }) {
  ...
  useEffect(() => {
    const fetchExamples = async () => {
      ...
      // Check cache first
      if (examplesCache.has(word.word_es)) {
        setExamples(examplesCache.get(word.word_es))
        setLoading(false)
      } else {
        try {
          const response = await api.getExamples(word.word_es)
          const data = response.data || []
          examplesCache.set(word.word_es, data)
          setExamples(data)
        } catch (e) {
      ...
      // Prefetch next word in background
      if (nextWord && !examplesCache.has(nextWord.word_es)) {
        try {
          const response = await api.getExamples(nextWord.word_es)
          const data = response.data || []
          examplesCache.set(nextWord.word_es, data)
        } catch (e) {
          // Silently fail on prefetch
          console.debug('Prefetch failed for next word:', e)
        }
      }
    }

    fetchExamples()
  }, [word.word_es, nextWord])
